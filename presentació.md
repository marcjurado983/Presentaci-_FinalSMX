# 🎤 Defensa Final del Projecte — Marc Jurado Valero

> **Escola Pia Santa Anna · SMX 2026**
> Defensa individual davant del tribunal · 15–20 minuts

---

## 👤 Presentació personal

Hola, soc en **Marc Jurado Valero**, estudiant de **Sistemes Microinformàtics i Xarxes** a l'Escola Pia Santa Anna de Mataró (promoció 2026).

Durant aquest curs he treballat en 8 projectes intermodulars, he fet pràctiques a **Merka360**, i he après a gestionar xarxes, sistemes, documentació tècnica i metodologies de treball com **Kanban** i **Gantt**.

- 📍 Mataró, Barcelona
- 📧 marcjurado00@gmail.com
- 🐙 [github.com/marcjurado983](https://github.com/marcjurado983)

---

## 🗂 Índex de la defensa

| # | Part | Contingut |
|---|------|-----------|
| 1 | [Perfil GitHub](#-part-1--presentació-del-perfil-github) | README, repositoris, estructura |
| 2 | [Anàlisi de projectes](#-part-2--anàlisi-de-projectes) | 2 projectes a l'atzar |
| 3 | [Defensa tècnica](#-part-3--defensa-tècnica) | Els 8 projectes del curs |
| 4 | [Metodologia](#-part-4--metodologia-de-treball) | Kanban, Gantt, GitHub |

---

## 🐙 Part 1 — Presentació del perfil GitHub

🔗 **Perfil:** [github.com/marcjurado983](https://github.com/marcjurado983)

### Què mostraré:
- ✅ README de perfil personalitzat
- ✅ Estructura general del perfil
- ✅ Tots els repositoris del curs
- ✅ Organització de carpetes i commits

### Repositoris principals:

| Repositori | Descripció |
|------------|-----------|
| [Projecte2](https://github.com/marcjurado983/Projecte2) | Consultoria EverPia |
| [Projecte3](https://github.com/marcjurado983/Projecte3) | Consultoria EverPia 2 |
| [Projecte4](https://github.com/marcjurado983/Projecte4) | Consultoria EverPia 3 |
| [EverPia-Onboarding](https://github.com/marcjurado983/EverPia-Onboarding) | Onboarding Escola Pia |
| [web-corporativa](https://github.com/marcjurado983/web-corporativa) | Web HTML/CSS |

---

## 🔍 Part 2 — Anàlisi de projectes

> El tribunal seleccionarà 2 projectes. Per a cadascun explicaré:

- 📁 Estructura del repositori
- 🎯 Objectiu i repte inicial
- 🛠 Decisions tècniques preses
- ⚠️ Problemes trobats i com es van resoldre
- 📄 Documentació i evidències generades
- 💡 Aprenentatges extrets

---

## 🛠 Part 3 — Defensa tècnica

Resum dels 8 projectes del curs:

### Projecte 01 — Arranquem 🚀
- **Repte:** Primer contacte amb l'entorn de treball i eines del curs.
- **Solució:** Configuració inicial de repositoris, GitHub i entorn de treball.
- **Aprenentatge:** Bases de Git, commits i estructura de projectes.

---

### Projecte 02 — Consultoria EverPia 📊
- **Repte:** Analitzar i proposar millores per a l'Escola Pia.
- **Solució:** Informe de consultoria amb diagnòstic i recomanacions.
- **Aprenentatge:** Documentació tècnica i anàlisi d'organitzacions.
- 🔗 [Repositori](https://github.com/marcjurado983/Projecte2)

---

### Projecte 03 — Consultoria EverPia 2 🎯
- **Repte:** Aprofundir en la consultoria i implementar millores.
- **Solució:** Pla d'acció amb fases i responsables.
- **Aprenentatge:** Planificació per fases i seguiment de tasques.
- 🔗 [Repositori](https://github.com/marcjurado983/Projecte3)

---

### Projecte 04 — Consultoria EverPia 3 💡
- **Repte:** Tancament i entrega final de la consultoria.
- **Solució:** Memòria final amb totes les evidències.
- **Aprenentatge:** Tancament de projectes i presentació de resultats.
- 🔗 [Repositori](https://github.com/marcjurado983/Projecte4)

---

### Projecte 05 — La incubadora 🌱
- **Repte:** Crear un projecte des de zero com si fos una startup.
- **Solució:** Definició d'idea, viabilitat i prototip.
- **Aprenentatge:** Metodologia àgil i treball en equip.

---

### Projecte 06 — Nexus 🔗
- **Repte:** Integració de sistemes i serveis en xarxa.
- **Solució:** Configuració de serveis interconnectats.
- **Aprenentatge:** Xarxes, serveis i resolució de problemes tècnics.

---

### Projecte 07 — Food Logístics S.A. 🚚
- **Repte:** Planificar la infraestructura IT d'una empresa logística.
- **Solució:** Proposta de xarxa, servidors i sistemes.
- **Aprenentatge:** Disseny d'infraestructures i justificació de decisions.

---

### Projecte 08 — Connecta't al Futur 📡
- **Repte:** Abordar un repte real de digitalització.
- **Solució:** Pla de digitalització amb eines actuals i IA.
- **Aprenentatge:** Transformació digital, IA aplicada i gestió del canvi.

---

## 📋 Part 4 — Metodologia de treball

### Kanban
Organització de tasques en 4 estats:

```
PENDENT → EN CURS → EN REVISIÓ → FET
```

**Exemple real:** Durant el Projecte 06 (Nexus), una tasca de configuració de serveis va passar de "pendent" a "en revisió" dues vegades per problemes de connectivitat. Es va reorganitzar el tauler per prioritzar la resolució del problema tècnic.

---

### Diagrama de Gantt
- Visualització de la **durada de cada tasca**
- Control de **dependències** entre fases
- Detecció de **desviacions** respecte al pla inicial

**Fases tipus d'un projecte:**
1. Anàlisi
2. Planificació
3. Desenvolupament
4. Proves
5. Documentació
6. Entrega

---

### GitHub com a eina de seguiment
- **Commits:** Registre de tots els canvis
- **README:** Documentació de cada projecte
- **Issues:** Seguiment de tasques i problemes
- **Branches:** Treball paral·lel sense afectar el codi principal

---

### Exemple real de gestió d'imprevistos

> Durant un dels projectes, una tasca que semblava senzilla (configuració d'un servei de xarxa) va trigar el doble del previst per un conflicte de ports. La solució va ser:
> 1. Identificar el problema al log del sistema
> 2. Reorganitzar les tasques al Kanban
> 3. Documentar la solució per evitar-ho en futurs projectes

---

## Conclusions i aprenentatges

### Tècnics
- Configuració de xarxes LAN/WAN i sistemes
- Administració Windows Server i Linux
- Ús de Git i GitHub per al control de versions
- Virtualització i gestió de serveis

### Metodològics
- Planificació amb Kanban i Gantt
- Documentació tècnica de qualitat
- Gestió d'imprevistos i problemes tècnics
- Treball en equip i comunicació

### Personals
- Millora de la capacitat d'autoaprenentatge
- Pràctiques professionals a Merka360
- Interès creixent per la IA i la ciberseguretat

---

## 📬 Contacte

| | |
|--|--|
| 📧 Email | marcjurado00@gmail.com |
| 🐙 GitHub | [github.com/marcjurado983](https://github.com/marcjurado983) |
| 📍 Ubicació | Mataró, Barcelona |

---

*Marc Jurado Valero · SMX 2026 · Escola Pia Santa Anna*
